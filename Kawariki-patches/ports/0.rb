...
module ErrorLogWriter
  ...
  def self.write(errobj)
    ...
    begin
      ...
      # Helper: convert any string-like object to UTF-8 safely
      to_utf8 = lambda do |s|
        s = "" if s.nil?
        str = s.to_s.dup
        # If already UTF-8 and valid, return normalized UTF-8
        ...
        # Try to convert from common Windows ANSI (CP932) to UTF-8, fallback to replacement
        begin
          return str.encode('UTF-8', 'CP932', invalid: :replace, undef: :replace, replace: '?')
        rescue Encoding::ConverterNotFoundError, Encoding::UndefinedConversionError
          begin
            return str.force_encoding('UTF-8')
          rescue
            return str.encode('UTF-8', invalid: :replace, undef: :replace, replace: '?') rescue str.to_s
          end
        end
      end

      File.open("errors.txt", "a:UTF-8") do |file|
        file.write("*Error - " + Time.now.strftime("%Y-%m-%d %H:%M:%S (%A)") + "\n")
        file.write("Exception : #{errobj.class}\n")
        file.write(to_utf8.call(errobj.message))

        unless $@.nil? || $@.empty?
          backtrace = ""
          $@.dup.each do |str|
            s = to_utf8.call(str)
            if (m = s.match(/^(?:Section)?{?(\d+)}?:/))
              begin
                idx = m[1].to_i
                extra = $RGSS_SCRIPTS.at(idx).at(1) rescue nil
                s = s.sub(/^(?:Section)?{?\d+}?:/) { "( " + (to_utf8.call(extra) || "unknown script") + " )：" }
              rescue
          ...
          file.write("\ntrace:\n" + to_utf8.call(backtrace) + "\n")
        end
      end
    rescue Exception => errs
      # Re-raise with combined message but ensure encodings are safe
      combined = "#{to_utf8.call(errs.message)}\n (#{to_utf8.call(errobj.class.to_s)})\n#{to_utf8.call(errobj.message)}"
      raise(errs, combined)
    end
  end
...
module MessageBox
  ...
  def self.error_message_setup(errobj)
    # Ensure graphics frozen for stable state
    Graphics.freeze
    begin
      Graphics.safe_update
    rescue SecurityHazard
    ...
    # Helper: safely convert from CP932/ASCII-8BIT to UTF-8
    ansi_to_utf8 = lambda do |s|
      next "" if s.nil?
      str = s.dup
      # If it's already UTF-8 and valid, return normalized UTF-8
      if str.encoding == Encoding::UTF_8
        begin
          return str.encode('UTF-8')
        rescue Encoding::UndefinedConversionError
          # fall through to force/encode below
        end
      end
      # If it's ASCII-8BIT or other, treat as CP932 (Windows ANSI Japanese) then convert
      begin
        str.force_encoding('CP932').encode('UTF-8', invalid: :replace, undef: :replace, replace: '?')
      rescue Encoding::ConverterNotFoundError
        # Fallback: try to treat as UTF-8 bytes
        str.force_encoding('UTF-8')
      end
    end

    # Helper: normalize any string to UTF-8 (keeps empty string for nil)
    to_utf8 = lambda do |s|
      next "" if s.nil?
    ...
    # Build backtrace and message safely
    unless $@.nil? || ($@.at(0)).nil?
      # Duplicate first trace element for tracer
      tracer = ($@.at(0)).dup rescue ""
      # Parse and rewrite Section{n}: patterns to script names if possible
      backtrace = ""
      $@.each do |str|
        s = str.dup rescue str.to_s
        # If the trace contains Section{n}: or Sectionn: replace with script name
        if (m = s.match(/^(?:Section)?{?(\d+)}?:/))
          begin
            idx = m[1].to_i
            extra = $RGSS_SCRIPTS.at(idx).at(1) rescue nil
            s.gsub!(/^(?:Section)?{?\d+}?:/) { "( " + (extra || "unknown script") + " )：" }
          rescue
            # ignore replacement errors
          end
        end
      ...
      # Prepare message body (avoid SystemStackError message duplication)
      unless errobj.is_a?(SystemStackError)
        if respond_to?(:rpgvxace?) && rpgvxace?
          # Some environments may have different encodings; force to UTF-8 safely
          _message = to_utf8.call(errobj.message.to_s) + "\n** backtrace：\n" + to_utf8.call(backtrace)
        else
          _message = errobj.message.to_s + "\n** backtrace：\n" + to_utf8.call(backtrace)
          # If errobj.message is not UTF-8, normalize it
          _message = to_utf8.call(_message)
        end
      end
    else
      tracer = "" # backtrace not available
      _message = respond_to?(:rpgvxace?) && rpgvxace? ? to_utf8.call(errobj.message.to_s) : errobj.message.to_s
      _message = to_utf8.call(_message)
    end

    # Collapse double CRLF sequences to single newline
    begin
      while _message.include?("\n\n")
        _message.gsub!("\n\n", "\n")
      end
    rescue Encoding::CompatibilityError
      # If gsub! fails due to encoding mismatch, ensure UTF-8 then retry
      _message = to_utf8.call(_message)
      _message.gsub!("\n\n", "\n") rescue nil
    end

    # Prepend explanatory header depending on debug mode
    header = if respond_to?(:debug?) && debug?
    "Unrecoverable exception " + (errobj.class).inspect + " occurred; cannot continue.\n"
  else
    "Unrecoverable exception " + (errobj.class).inspect +
             " occurred; processing could not continue.\n" +
             "We apologize for the inconvenience.\n\nDetails:\n"
  end

  # Ensure header and message are UTF-8 and safe to concatenate
  header = to_utf8.call(header)
  _message = to_utf8.call(_message)

  full_message = header + _message

  # Normalize tracer to UTF-8 as well
  tracer = to_utf8.call(tracer)

  # Final cleanup: remove any embedded NULs that might come from Win32 buffers
  begin
    full_message.tr!("\x00", "")
    tracer.tr!("\x00", "")
  rescue Encoding::CompatibilityError
    full_message = to_utf8.call(full_message).tr("\x00", "")
    tracer = to_utf8.call(tracer).tr("\x00", "")
  end

  return full_message, tracer
end
