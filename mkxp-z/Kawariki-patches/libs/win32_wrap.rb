...
class Win32API
	...
	def initialize(dll, func, *args)
		...
		if !System.is_windows? || !NATIVE_ON_WINDOWS
			if Win32API_Impl.const_defined?(dll)
				...
				if dll_impl.const_defined?(func)
					...
					puts "[Win32API] Using wrapped implementation for #{@dll}:#{@func}" if LOG_NATIVE
					return
				end
			end
		end
		...
		begin
			...
			puts "[Win32API] Native initialization successful for #{@dll}:#{@func}" if LOG_NATIVE
		rescue => e
			puts "[Win32API] Error during native initialization: #{e.message}" if LOG_NATIVE
		end
	end
	...
	def call(*args)
		if @mkxp_wrap_impl
			log_call("Calling wrapped implementation", args)
			return @mkxp_wrap_impl.call(args)
		end

		if @mkxp_native_available
			log_call("Calling native function", args)
			return mkxp_native_call(*args)
		end

		handle_error("Function #{@dll}:#{@func} not available")
	end

	private

	def log_call(action, args)
		puts "[Win32API] #{action} for #{@dll}:#{@func} with args: #{args.inspect}" if LOG_NATIVE
	end

	def handle_error(message)
		if TOLERATE_ERRORS
			puts "[Win32API] #{message}, returning 0" if !@called
			@called = true
			return 0
		else
			puts "[Win32API] #{message}, raising RuntimeError"
